...
def fetch_insights() -> dict:
    url = f"{config.GLEAN_INSTANCE_URL.rstrip('/')}/rest/api/v1/insights"
    headers = {
        "Authorization": f"Bearer {config.GLEAN_API_TOKEN}",
        "Content-Type": "application/json",
    }
    day_range = {
        "start": {"daysFromNow": config.DAYS_FROM_NOW_START},
        "end":   {"daysFromNow": config.DAYS_FROM_NOW_END},
    }
    body = {
    ...
    log.info("Fetching Glean insights (last %d days)...", config.DAYS_FROM_NOW_START)
...
def _base_meta(ingested_at: str) -> dict:
    return {
        ...
        "period_days": config.DAYS_FROM_NOW_START,
    }
...
def main() -> None:
    ...
    es = get_es_client()
    for index, docs in [
        (config.INDEX_OVERVIEW,   overview_docs),
        (config.INDEX_ASSISTANT,  assistant_docs),
        (config.INDEX_AGENTS,     agents_docs),
    ]:
